...
fields = [:first_name, :last_name, :email, :username, :total_karma, :created_at, :updated_at]
TOTAL_USERS.times.each_slice(SLICE_SIZE).each_with_index do |ids, index|
  data = ids.map do |i|
    [Faker::Name.first_name, Faker::Name.last_name, "email_#{i}@example.com", "user_#{i}", 0,now, now]
  end
